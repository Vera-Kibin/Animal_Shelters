...
import { useState } from "react";
import CommentModal from "./CommentModal";
import { Dog, Cat, Paw } from "../../assets/Animals";
import "./ShelterCard.css";
...
const Icon = {
  phone: (
    <svg viewBox="0 0 24 24" width="14" height="14" fill="currentColor">
      <path d="M6.6 10.8a15.5 15.5 0 006.6 6.6l2.2-2.2a1 1 0 011-.24 11 11 0 003.4.55 1 1 0 011 1V20a1 1 0 01-1 1A17 17 0 013 4a1 1 0 011-1h3.5a1 1 0 011 1 11 11 0 00.55 3.4 1 1 0 01-.24 1z" />
    </svg>
  ),
  globe: (
    <svg
  ...
  fb: (
    <svg viewBox="0 0 24 24" width="15" height="15" fill="currentColor">
      <path d="M14 9h3V6h-3c-2 0-3 1-3 3v2H9v3h2v6h3v-6h2.5l.5-3H14V9z" />
    </svg>
  ),
  ig: (
    <svg
...
export default function ShelterCard({ shelter, query, selected, onSelect }) {
  ...
  return (
    ...
    >
      <div className="shelter-card__head">
        <h3 className="shelter-card__name">
          <Highlight text={name} query={query} />
        </h3>
        {location?.city && (
          <span className="shelter-card__city">
            <Highlight text={location.city} query={query} />
          </span>
        )}
      </div>
      {typeof shelter._distanceKm === "number" && (
        <span className="shelter-card__dist">
          ~{shelter._distanceKm.toFixed(1)} km od Ciebie
        </span>
      )}

      {location?.address && (
        <p className="shelter-card__addr">{location.address}</p>
      )}

      <div className="shelter-card__tags">
        {(details?.animalTypes || []).slice(0, 4).map((t) => {
          const Icon = t === "dogs" ? Dog : t === "cats" ? Cat : Paw;
          return (
            <span key={t} className="shelter-card__tag">
              <Icon className="shelter-card__tagicon" />
              {ANIMAL_LABELS[t] || t}
            </span>
          );
        })}
      ...
      {phones.length > 0 && (
        <div className="shelter-card__phones">
          ...
          {phones.length > 1 && (
            <button
              className="shelter-card__more"
              onClick={(e) => {
                stop(e);
                setShowAllPhones((v) => !v);
              }}
            >
              {showAllPhones
                ? "ukryj numery"
                : "+" + (phones.length - 1) + " więcej numerów"}
            </button>
          )}
        </div>
      )}
