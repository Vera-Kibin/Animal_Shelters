...
export default function ShelterCard({ shelter, query, selected, onSelect, onOpenProfile }) {
  ...
  return (
    ...
    >
      ...
      {typeof shelter._distanceKm === "number" && (
        <span className="shelter-card__dist">~{shelter._distanceKm.toFixed(1)} km od Ciebie</span>
      )}
      ...
      <div className="shelter-card__links">
        ...
        <button
          className="shelter-card__profile"
          onClick={(e) => { stop(e); onOpenProfile?.(shelter); }}
        >
          Zobacz profil →
        </button>
      </div>
